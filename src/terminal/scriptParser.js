...
// - Indentation-aware parsing producing AST
// - Function calls like move("south", 3) and move('east', 3)
// - Fallback tokens like move east 3 and single-token directions
 ...
 * Produces tokens from source code with indentation tracking
 */
class Lexer {
    constructor(source) {
        this.source = source;
 ...
 * Supports function calls, fallback tokens, sequences, and indentation blocks
 */
class Parser {
    constructor(tokens) {
        this.tokens = tokens;
    ...
    parseStatement() {
        ...
        if (token.type === TokenType.IDENTIFIER) {
            return this.parseCallOrFallback();
        }
    ...
    /**
     * Parse a function call or fallback token syntax
     * Supports: move("east", 3) OR move east 3 OR east
     */
    parseCallOrFallback() {
        const calls = [];
        calls.push(this.parseCallOrFallbackSingle());
    ...
    /**
     * Parse a single function call or fallback token
     */
    parseCallOrFallbackSingle() {
        const nameToken = this.advance();
        const name = nameToken.value;
        
        // Check if it's a proper function call with parentheses
        if (this.peek().type === TokenType.LPAREN) {
            return this.parseCallWithParens(name, nameToken.line);
        }
        
        // Fallback: check if this is a simple direction command (like "east" or "north")
        const directions = ['north', 'south', 'east', 'west'];
        const lowerName = name.toLowerCase();
        
        if (directions.includes(lowerName)) {
            // Single direction token - convert to move("direction")
            return {
                type: NodeType.FALLBACK_CALL,
                name: 'move',
                arguments: [{ type: NodeType.STRING, value: lowerName }],
                line: nameToken.line
            };
        }
        
        // Fallback: identifier followed by tokens without parentheses
        // e.g., "move east 3" -> move("east", 3)
        const args = [];
        
        // Collect arguments until newline, comma, colon, or dedent
        while (this.peek().type !== TokenType.NEWLINE &&
            ...
            if (argToken.type === TokenType.IDENTIFIER) {
                this.advance();
                args.push({ type: NodeType.STRING, value: argToken.value });
            } else if (argToken.type === TokenType.NUMBER) {
                this.advance();
                args.push({ type: NodeType.NUMBER, value: argToken.value });
            } else if (argToken.type === TokenType.STRING) {
                this.advance();
                args.push({ type: NodeType.STRING, value: argToken.value });
            } else {
                break;
        ...
        if (args.length === 0) {
            // Simple function call without arguments: feed -> feed()
            return {
                type: NodeType.CALL,
                name,
                arguments: [],
    ...
    parseWhile() {
        ...
        if (this.peek().type !== TokenType.INDENT) {
            throw new SyntaxError(`Expected indented block after while at line ${whileToken.line}`);
        }
        this.advance(); // consume INDENT
 ...
 * Provides visual feedback during execution with configurable delays
 */
class Executor {
    constructor(commandHandlers, printFn, options = {}) {
        this.commands = commandHandlers;
        this.print = printFn || console.log;
        this.executionDelay = options.executionDelay || 200; // ms between commands
        this.maxIterations = options.maxIterations || 1000; // Safety limit for loops
        this.stopped = false;
    }
     ...
     * Used for evaluating conditions in if/while statements.
     */
    ...
    async executeIf(node) {
        ...
        if (conditionResult) {
            ...
            this.print(`✗ Condition is false, executing else block`, 'info');
    ...
    async executeWhile(node) {
        const { condition, body, maxIterations } = node;
        const limit = maxIterations || this.maxIterations;
        let iterations = 0;
        
        this.print(`🔄 Starting while loop...`, 'info');
        
        while (this.evaluateCondition(condition) && iterations < limit) {
            if (this.stopped) break;
            iterations++;
        ...
        if (iterations >= limit) {
            this.print(`⚠️ Loop stopped after ${limit} iterations (safety limit)`, 'error');
        }
    }
...
function isMultiLineScript(source) {
    ...
    // Check for newlines, repeat, if, or while statements
    return trimmed.includes('\n') || 
           /^repeat\s*\(/i.test(trimmed) ||
           /^if\s+/i.test(trimmed) ||
           /^while\s+/i.test(trimmed);
}
 ...
 * @param {object} options - Optional configuration (executionDelay, maxIterations)
 * @returns {Executor} - An executor instance
 */
function createExecutor(commands, printFn, options) {
    return new Executor(commands, printFn, options);
}

/**
 * Parse and execute a script in one call
 * @param {string} source - The source code to execute
 * @param {object} commands - Command handlers
 * @param {function} printFn - Print function
 * @param {object} options - Executor options
 * @returns {Promise} - Resolves when execution completes
 */
async function executeScript(source, commands, printFn, options) {
    const ast = parseScript(source);
    const executor = createExecutor(commands, printFn, options);
